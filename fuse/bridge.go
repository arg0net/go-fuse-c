...
func ll_SetXAttr(id C.int, ino C.fuse_ino_t, name *C.char, value unsafe.Pointer,
	...
	err := fs.SetXAttr(InodeID(ino), C.GoString(name), data, int(flags))
	return C.int(err)
}
...
func ll_GetXAttr(id C.int, ino C.fuse_ino_t, name *C.char, buf unsafe.Pointer,
	...
	var outSize int
	if *size == 0 {
		outSize, err = fs.GetXAttrSize(InodeID(ino), C.GoString(name))
	} else {
		out := zeroCopyBuf(buf, int(*size))
		outSize, err = fs.GetXAttr(InodeID(ino), C.GoString(name), out)
	}
	if err == OK {
...
func ll_Lookup(id C.int, dir C.fuse_ino_t, name *C.char,
	...
	fs := getFS(int(id))
	ent, err := fs.Lookup(InodeID(dir), C.GoString(name))
	if err == OK {
		ent.toCEntry(cent)
	}
	return C.int(err)
...
func ll_GetAttr(id C.int, ino C.fuse_ino_t, fi *C.struct_fuse_file_info,
	...
	fs := getFS(int(id))
	attr, err := fs.GetAttr(InodeID(ino), newFileInfo(fi))
	if err == OK {
...
func ll_SetAttr(id C.int, ino C.fuse_ino_t, attr *C.struct_stat, toSet C.int,
	...
	var ia InoAttr
	ia.fromCStat(attr)
	oattr, err := fs.SetAttr(InodeID(ino), &ia, SetAttrMask(toSet), newFileInfo(fi))
	if err == OK {
...
func ll_ReadDir(id C.int, ino C.fuse_ino_t, size C.size_t, off C.off_t,
	...
	fs := getFS(int(id))
	writer := &dirBuf{db}
	err := fs.ReadDir(InodeID(ino), newFileInfo(fi), int64(off), int(size), writer)
	return C.int(err)
}
...
func ll_OpenDir(id C.int, ino C.fuse_ino_t, fi *C.struct_fuse_file_info) C.int {
	fs := getFS(int(id))
	info := newFileInfo(fi)
	err := fs.OpenDir(InodeID(ino), info)
	if err == OK {
		fi.fh = C.uint64_t(info.Handle)
	}
	return C.int(err)
...
func ll_Release(id C.int, ino C.fuse_ino_t, fi *C.struct_fuse_file_info) C.int {
	fs := getFS(int(id))
	err := fs.Release(InodeID(ino), newFileInfo(fi))
	return C.int(err)
}
...
func ll_ReleaseDir(id C.int, ino C.fuse_ino_t, fi *C.struct_fuse_file_info) C.int {
	fs := getFS(int(id))
	err := fs.ReleaseDir(InodeID(ino), newFileInfo(fi))
	return C.int(err)
}
...
func ll_FSync(id C.int, ino C.fuse_ino_t, datasync C.int, fi *C.struct_fuse_file_info) C.int {
	...
	err := fs.FSync(InodeID(ino), dataOnly, newFileInfo(fi))
	return C.int(err)
}
...
func ll_FSyncDir(id C.int, ino C.fuse_ino_t, datasync C.int, fi *C.struct_fuse_file_info) C.int {
	fs := getFS(int(id))
	var dataOnly bool = datasync != 0
	err := fs.FSyncDir(InodeID(ino), dataOnly, newFileInfo(fi))
	return C.int(err)
}
...
func ll_Flush(id C.int, ino C.fuse_ino_t, fi *C.struct_fuse_file_info) C.int {
	fs := getFS(int(id))
	err := fs.Flush(InodeID(ino), newFileInfo(fi))
	return C.int(err)
}
...
func ll_Read(id C.int, req C.fuse_req_t, ino C.fuse_ino_t, size C.size_t, off C.off_t,
	...
	// Create slice backed by C buffer.
	buf, err := fs.Read(InodeID(ino), int64(size), int64(off), newFileInfo(fi))
	if err != OK {
		return C.int(err)
	}
...
func ll_Write(id C.int, ino C.fuse_ino_t, buf unsafe.Pointer, n *C.size_t, off C.off_t,
	...
	written, err := fs.Write(in, InodeID(ino), int64(off), newFileInfo(fi))
	if err == OK {
		*n = C.size_t(written)
	}
	return C.int(err)
...
func ll_Mknod(id C.int, dir C.fuse_ino_t, name *C.char, mode C.mode_t,
	...
	fs := getFS(int(id))
	ent, err := fs.Mknod(InodeID(dir), C.GoString(name), int(mode), int(rdev))
	if err == OK {
		ent.toCEntry(cent)
	}
	return C.int(err)
...
func ll_ListXAttr(id C.int, ino C.fuse_ino_t, buf unsafe.Pointer, size *C.size_t) C.int {
	out := zeroCopyBuf(buf, int(*size))
	fs := getFS(int(id))
	keys, err := fs.ListXAttrs(InodeID(ino))
	if err != OK {
		return C.int(err)
	}
...
func ll_RemoveXAttr(id C.int, ino C.fuse_ino_t, name *C.char) C.int {
	fs := getFS(int(id))
	err := fs.RemoveXAttr(InodeID(ino), C.GoString(name))
	return C.int(err)
}
...
func ll_Access(id C.int, ino C.fuse_ino_t, mask C.int) C.int {
	fs := getFS(int(id))
	return C.int(fs.Access(InodeID(ino), int(mask)))
}
...
func ll_Create(id C.int, dir C.fuse_ino_t, name *C.char, mode C.mode_t,
	...
	fs := getFS(int(id))
	info := newFileInfo(fi)
	ent, err := fs.Create(InodeID(dir), C.GoString(name), int(mode), info)
	if err == OK {
		ent.toCEntry(cent)
		fi.fh = C.uint64_t(info.Handle)
	}
...
func ll_Mkdir(id C.int, dir C.fuse_ino_t, name *C.char, mode C.mode_t,
	...
	fs := getFS(int(id))
	ent, err := fs.Mkdir(InodeID(dir), C.GoString(name), int(mode))
	if err == OK {
		ent.toCEntry(cent)
	}
	return C.int(err)
...
func ll_Rmdir(id C.int, dir C.fuse_ino_t, name *C.char) C.int {
	fs := getFS(int(id))
	err := fs.Rmdir(InodeID(dir), C.GoString(name))
	return C.int(err)
}
...
func ll_Symlink(id C.int, link *C.char, parent C.fuse_ino_t, name *C.char,
	cent *C.struct_fuse_entry_param) C.int {
	fs := getFS(int(id))
	ent, err := fs.Symlink(C.GoString(link), InodeID(parent), C.GoString(name))
	if err == OK {
		ent.toCEntry(cent)
	}
	return C.int(err)
...
func ll_Link(id C.int, ino C.fuse_ino_t, newparent C.fuse_ino_t, name *C.char,
	cent *C.struct_fuse_entry_param) C.int {
	fs := getFS(int(id))
	ent, err := fs.Link(InodeID(ino), InodeID(newparent), C.GoString(name))
	if err == OK {
		ent.toCEntry(cent)
	}
	return C.int(err)
...
func ll_Unlink(id C.int, dir C.fuse_ino_t, name *C.char) C.int {
	fs := getFS(int(id))
	err := fs.Unlink(InodeID(dir), C.GoString(name))
	return C.int(err)
}
...
func ll_Rename(id C.int, dir C.fuse_ino_t, name *C.char,
	...
	fs := getFS(int(id))
	err := fs.Rename(InodeID(dir), C.GoString(name), InodeID(newdir), C.GoString(newname))
	return C.int(err)
}
...
func (d *dirBuf) Add(name string, ino InodeID, mode int, next InodeID) bool {
	cstr := C.CString(name)
